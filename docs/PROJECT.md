...
Check-in eligibility is evaluated again at the database write boundary. The API returns stable conflict codes for inactive, closed, full, duplicate-active, and concurrent check-in outcomes; a closed response may include the parking-local next opening timestamp. A rejected check-in does not create a vehicle or change its stable metadata.
