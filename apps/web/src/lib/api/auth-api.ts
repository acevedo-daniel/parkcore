...
export async function loginDemo(): Promise<AuthResponse> {
  ...
  if (data) return data;
  throw new ApiError(
    getApiErrorMessage(error, 'Demo access is unavailable.'),
    response.status,
    getApiErrorCode(error),
  );
}
...
export async function resetDemo() {
  ...
  if (data) return data;
  throw new ApiError(
    getApiErrorMessage(error, 'Unable to reset the demo.'),
    response.status,
    getApiErrorCode(error),
  );
}
...
export async function getCurrentUser(): Promise<User> {
  ...
  if (data) return data;
  throw new ApiError(
    getApiErrorMessage(error, 'Unable to restore your session.'),
    response.status,
    getApiErrorCode(error),
  );
}
