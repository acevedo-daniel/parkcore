...
- Keep route modules lazy and use URL search parameters for catalog/history filters and pagination.
- Call the backend only through `@parkcore/api-client`; do not import from `apps/api` or duplicate HTTP contract types.
- Configure authentication via the API client's token callback. Browser JWT persistence belongs in `apps/web`, not the generated client.
...
- Keep the Playwright owner workflow passing; it runs against mocked contract-shaped HTTP responses and needs no backend process.
